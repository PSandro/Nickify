...
public final class PresetsInventory extends AbstractStaticInventory {
    ...
    static {
        final ItemStack messagesStack = new ItemBuilder(Material.IRON_CHESTPLATE)
                ...
                .build();
        TEAM_VIEW_ITEM = new ClickableItem(messagesStack, (player, clickType, context, inventory) -> {
            inventory.closeSilent(player);
            context.createInventory(player, TeamViewPresetsInventory.class, inventory).open(player);
        });
        ...
        final ItemStack settingsStack = new ItemBuilder(Material.NAME_TAG)
                ...
                .build();
        NICK_ITEM = new ClickableItem(settingsStack, (player, clickType, context, inventory) -> {
            inventory.closeSilent(player);
        });
    }

    public PresetsInventory(AbstractDynamicInventory parent, InventoryActionCallback inventoryActionCallback) {
        super(parent, "§6Nickify Presets", 45, inventoryActionCallback);
    }
